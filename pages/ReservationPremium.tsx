            ...
            <div className="sticky top-[110px] border border-[#ddd] bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="max-h-[80vh] overflow-y-auto no-scrollbar">
                    <div className={`border-b border-[#eee] ${openSection === 'date' ? 'bg-white' : 'bg-[#f9f9f9]'}`}>
                        ...
                        {openSection === 'date' && <div className="px-5 pb-6 bg-white"><div className="grid grid-cols-7 gap-1">{CALENDAR_DAYS.map(day => (<button key={day} onClick={() => { setSelectedDate(`${_calYear}-${_calMonthStr}-${day.toString().padStart(2,'0')}`); setOpenSection('options'); }} className={`h-9 text-[13px] rounded hover:bg-gray-50 flex items-center justify-center ${selectedDate?.includes(day.toString().padStart(2,'0')) ? 'bg-[#0070F0] text-white font-bold' : 'text-[#333]'}`}>{day}</button>))}</div></div>}
                    </div>
                                    ...
                                    <div key={guest.id} className="bg-[#FFFBE6] p-4 rounded-xl border border-[#FFE58F] relative">
                                        ...
                                        <div className="space-y-2">
                                            <div className="grid grid-cols-2 gap-2">
                                                ...
                                                <div>
                                                    <label className="text-[10px] font-bold text-gray-400 block mb-0.5">{t('gender')}</label>
                                            ...
                                            {idx === 0 && (
                                                <div className="pt-2 border-t border-[#FFE58F] mt-2">
                                                    <label className="text-[10px] font-bold text-[#C8A32B] block mb-1 flex items-center gap-1"><MessageCircle size={10}/> {t('representative_contact')}</label>
                        ...
                        <>
                            ...
                            {!showPayPal ? (
                                ...
                                <div className="space-y-3 animate-fade-in">
                                    ...
                                    <PayPalCheckout amount={krwToUsd(getPrice(), exchangeRate)} description={`Premium Package - ${selectedDate}`} items={[{ name: 'Premium Health Checkup Package', price: krwToUsd(getPrice() / guestList.length, exchangeRate), quantity: guestList.length }]} onSuccess={handlePayPalSuccess} onError={(err) => { alert(isEn ? 'Payment error.' : '결제 오류가 발생했습니다.'); console.error(err); }} onCancel={() => setShowPayPal(false)} language={language}/>
