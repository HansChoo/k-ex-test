...
interface ReservationBasicProps {
  language: 'ko' | 'en'; // Kept for compat but we use Context
}
...
export const ReservationBasic: React.FC<ReservationBasicProps> = () => {
  ...
  const DETAIL_IMAGES = {
    ...
    intro: "https://ecimg.cafe24img.com/pg2441b44963288024/samsongenm1/web/upload/NNEditor/20260123/c8bacd4323350a63de2418a099f697.png",
    ...
    glassSkinMain: "https://ecimg.cafe24img.com/pg2441b44963288024/samsongenm1/web/upload/NNEditor/20260123/9c1296c05c003793087dffe5e67613c4.png",
  };
        ...
        <div className="flex-1 w-full min-w-0">
            <div className="px-4 lg:px-0 mb-8">
                ...
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-4 text-[13px] lg:text-[14px] font-bold text-[#555]">
                        <button className="flex items-center gap-1.5 hover:text-blue-600"><Heart size={18} /><span>{t('wishlist')}</span></button>
                        <span className="w-px h-3 bg-gray-300"></span>
                        <button className="flex items-center gap-1.5 hover:text-blue-600"><MapPin size={18} /><span>{t('map')}</span></button>
                    </div>
                </div>
            </div>
            ...
            <div className="mb-12">
                <div className="relative w-full bg-gray-50 rounded-none lg:rounded-2xl overflow-hidden mb-3 group aspect-square lg:aspect-[1.2/1]">
                    ...
                    <button onClick={() => setCurrentImageIndex(prev => prev > 0 ? prev - 1 : prev)} className={`absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full flex items-center justify-center shadow-sm opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-0`} disabled={currentImageIndex === 0}><ChevronLeft size={20} /></button>
                    <button onClick={() => setCurrentImageIndex(prev => prev < IMAGES.length - 1 ? prev + 1 : prev)} className={`absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full flex items-center justify-center shadow-sm opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-0`} disabled={currentImageIndex === IMAGES.length - 1}><ChevronRight size={20} /></button>
                </div>
            </div>
